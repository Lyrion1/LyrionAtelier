// netlify/functions/printful-sync.js
import { readFileSync } from 'fs';

const fetchApi = globalThis.fetch;
const EMPTY_FRONT_SET = new Set();

export async function handler(event) {
  try {
    if (!fetchApi) return json(500, { error: 'Fetch API unavailable in this runtime' });
    ...
    const frontSets = {
      adult: new Set(brandCfg?.fronts?.adult || []),
      youth: new Set(brandCfg?.fronts?.youth || [])
    };
    const frontSetCombined = new Set([...frontSets.adult, ...frontSets.youth]);
    ...
    for (const slug of picked) {
      ...
      const pxFront = Math.round(frontWidthIn(prod, brandCfg, frontSetCombined) * 300);
      ...
      for (const v of prod.variants || []) {
        ...
        if (existBySku[payload.sku]) {
          await pfPUT(`${baseApi}/store/variants/${existBySku[payload.sku]}`, key, payload);
          updatedN++;
        } else {
          await pfPOST(`${baseApi}/store/variants`, key, { sync_product_id: productId, ...payload });
          createdN++;
        }
      }
...
function tryRead(f) { try { return JSON.parse(readFileSync(f, 'utf8')); } catch { return null; } }
async function head(url) { try { const r = await fetchApi(url, { method: 'HEAD' }); return r.ok; } catch { return false; } }
function centsToPrice(c) { return (Number(c || 0) / 100).toFixed(2); }
function file(type, url) { return { type, url }; }
function frontWidthIn(p, cfg, frontSet) {
  const sub = (p.subcategory || '').toLowerCase();
  let w = 12;
  if (sub.includes('crew')) w = 11.5;
  if (sub.includes('hood')) w = 12;
  const fronts = frontSet || EMPTY_FRONT_SET;
...
async function ensureSyncProduct(prod, key, baseApi, slug) {
  ...
  if (id) {
    await pfPUT(`${baseApi}/store/products/${id}`, key, { sync_product: { name: prod.title, external_id: slug } })
      .catch(err => console.warn('[printful-sync] update product failed', slug, err?.message || err));
...
async function pfGET(url, key) {
  const r = await fetchApi(url, { headers: { Authorization: `Bearer ${key}` } });
  if (!r.ok) throw new Error('GET ' + r.status + ' ' + url);
  return r.json();
}
async function pfPOST(url, key, body) {
  ...
  if (!r.ok) {
    ...
    throw new Error('POST ' + r.status + ' ' + url + ' ' + t);
  }
  return r.json();
}
async function pfPUT(url, key, body) {
  const r = await fetchApi(url, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` },
    body: JSON.stringify(body)
  ...
  if (!r.ok) {
    const t = await r.text();
    throw new Error('PUT ' + r.status + ' ' + url + ' ' + t);
  }
  return r.json();
}
